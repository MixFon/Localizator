...
## Подкоманды (режимы CLI)

| Подкоманда | Название в CLI | Назначение |
|------------|----------------|------------|
| Локализация | `localize` | Полный цикл по Swift-исходникам (подкоманда по умолчанию) |
| Дубликаты | `duplicates` | Проверка JSON на дубли и похожие ключи |
| Актуализация | `actualization` | Добавление недостающих case в `MMTranslationKeys` по JSON |

Вызов без имени подкоманды запускает **`localize`**.

Краткая справка:

```bash
Localizator --help
Localizator localize --help
Localizator duplicates --help
Localizator actualization --help
```
